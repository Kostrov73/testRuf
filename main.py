...
def show_popup():
    show = Popups()
    popupWindow = Popup(title ="Введите коректные данные", content = show,
                        size_hint =(None, None), size =(200, 95))
...
class FirstScr(Screen):
    def __init__(self,name='first'):
        ...
        with self.canvas.before:
            self.bg = Rectangle(pos=self.pos, size=self.size, source='Back.jpg')
            # self.bg = Rectangle(pos=self.pos, size=self.size, source='Back_edited.jpg')
        ...
        self.name_val=TextInput(text='', multiline=False, size_hint=(.75,.7), pos_hint={"top":.9,'center_x':.5})
        self.age_val=TextInput(text='7', multiline=False, size_hint=(.75,.7), pos_hint={"top":.9,'center_x':.5})
    ...
    def next(self):
        try:
            if int(self.age_val.text)>=7 and self.name_val.text!='':
                global age, name
...
class SecondScr(Screen):
    def __init__(self, name='second'):
        ...
        self.ti=TextInput(text='0', multiline=False, size_hint=(.75,1), pos_hint={"top":1.3,'center_x':.5})
...
class FourthScr(Screen):
    ...
    def next(self):
        ...
        if self.next_screen == False:
            ...
            try:
                if int(self.res1_value.text)>0 and int(self.res2_value.text)>0:
...
class FivethScr(Screen):
    ...
    def next(self):
        ...
        FirstScr.next_screen=False
        SecondScr.next_screen=False
        ThirdScr.next_screen=False
        FourthScr.next_screen=False
        FivethScr.next_screen=False
